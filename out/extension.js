...
const phosphorIconsGallery_1 = require("./phosphorIconsGallery");
...
function activate(context) {
    ...
    const snippetsTreeView = vscode.window.createTreeView("rajaSnippetsExplorer", {
    ...
    */
    ...
    const forceCollapseCommand = vscode.commands.registerCommand("rajaSnippets.forceCollapse", () => {
        vscode.commands.executeCommand("workbench.actions.treeView.rajaSnippetsExplorer.collapseAll");
        vscode.window.showInformationMessage("Force collapsed all snippet groups");
    });
    context.subscriptions.push(forceCollapseCommand);
    // Add auto-collapse toggle command
    let autoCollapseEnabled = false;
    const toggleAutoCollapseCommand = vscode.commands.registerCommand("rajaSnippets.toggleAutoCollapse", () => {
        autoCollapseEnabled = !autoCollapseEnabled;
        vscode.window.showInformationMessage(`Auto-collapse groups: ${autoCollapseEnabled ? "Enabled" : "Disabled"}`);
    });
    context.subscriptions.push(toggleAutoCollapseCommand);
    // Simple auto-collapse logic (when enabled)
    const expansionDisposable = snippetsTreeView.onDidExpandElement(async (event) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("rajaSnippets.refresh", () => treeProvider.refresh()), vscode.commands.registerCommand("rajaSnippets.addSnippetToGroup", async (groupItem) => {
        ...
        const groupName = typeof groupItem.label === "string"
            ? groupItem.label
      ...
      <body>
        ...
        <select id="group">
          ${manager
            .getData()
            .groups.map((g) => `<option value="${escapeHtml(g)}" ${g === newSnippet.group ? "selected" : ""}>${escapeHtml(g)}</option>`)
            .join("")}
        </select>
    ...
    context.subscriptions.push(vscode.commands.registerCommand("rajaSnippets.addGroup", async () => {
        ...
        if (item?.snippet) {
            ...
            const pick = await vscode.window.showQuickPick(codeSnippets.map((s) => ({
                label: s.title,
                description: s.group,
                snippet: s,
        ...
        if (item?.snippet) {
            ...
            const pick = await vscode.window.showQuickPick(terminalSnippets.map((s) => ({
                label: s.title,
                description: s.group,
                snippet: s,
      ...
      <body>
        ...
        <select id="group">
          ${manager
            .getData()
            .groups.map((g) => `<option value="${escapeHtml(g)}" ${g === snippet.group ? "selected" : ""}>${escapeHtml(g)}</option>`)
            .join("")}
        </select>
    ...
    vscode.commands.registerCommand("rajaSnippets.showGroupActions", async (item) => {
        const groupName = typeof item.label === "string"
            ? item.label
            : item.label.label;
        const picks = [
            {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("rajaSnippets.openStorage", async () => {
        ...
        try {
            ...
            const action = await vscode.window.showQuickPick([
                {
                    label: "Merge",
                    description: "Gabungkan dengan snippets yang ada",
                },
                {
                    label: "Replace",
    ...
    let showGallery = vscode.commands.registerCommand("rajaIcons.showGallery", async () => {
        const iconData = await (0, phosphorIconsGallery_1.loadPhosphorIcons)();
        vscode.window.showInformationMessage(`Loaded ${iconData.icons.length} Phosphor icons`);
        (0, phosphorIconsGallery_1.showIconsGallery)(iconData);
    });
    ...
    context.subscriptions.push(vscode.commands.registerCommand("rajaIcons.copyIcon", async (iconName, weight = "regular") => {
        if (iconName) {
            const identifier = (0, phosphorIconsTree_1.getIconIdentifier)(iconName, weight);
...
function foldByIndentation(minIndent) {
    ...
    for (let i = 0; i < document.lineCount; i++) {
        ...
        if (line.firstNonWhitespaceCharacterIndex >= minIndent &&
